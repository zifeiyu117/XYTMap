...
@interface XYTWeatherViewController ()<UINavigationControllerDelegate,NSURLSessionDelegate>

@property (nonatomic, strong) UIImageView  *backgroudView;
@property (nonatomic, strong) CLLocation *userLocation;
//@property (nonatomic, strong) Alamofire *manager;
...
@implementation XYTWeatherViewController
...
- (WeatherView *)weatherV {
    if (!_weatherV) {
        _weatherV = [[WeatherView alloc]initWithFrame:CGRectMake(0, 64, kScreenWidth, kScreenHeight-64)];
    }
    return _weatherV;
}
...
-(UIButton *)changeCityBtn {
    if (!_changeCityBtn) {
        ...
        _changeCityBtn.frame = CGRectMake(0, 0, 44, 44);
...
- (void)viewDidLoad {
    ...
    self.title=@"最近天气情况";
//    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithCustomView:self.changeCityBtn];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"location_hardware"] style:(UIBarButtonItemStylePlain) target:self action:@selector(changeCity)];
...
- (void)sendRequestToServer:(NSString *)cityName {
    ...
    NSURLSessionTask*dataTask = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        if (!error) {
            NSDictionary *resposeDict = [NSJSONSerialization JSONObjectWithData:data options:kNilOptions error:nil];
            NSArray *resultArray = resposeDict[@"results"];
